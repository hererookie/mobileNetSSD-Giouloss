...
def log_sum_exp(x):
    ...
    """
    x_max = x.data.max()
    # return torch.log(torch.sum(torch.exp(x - x_max), 1, keepdim=True) + 1e-6) + x_max
...
def decode(loc, priors, variances):
    ...
    # print("loc[:, 2:] * variances[1]:", loc[:, 2:] * variances[1])
    # print("loc[:, 2:] * variances[1].shape:", loc[:, 2:] * variances[1].shape)
    #
    # print("torch.exp(loc[:, 2:]* variances[1]).shape:", torch.exp(loc[:, 2:] * variances[1]).shape)
    # print("**************************************")
    boxes = torch.cat((
...
def intersect(box_a, box_b):
    ...
    Args:
      ...
      A: 24
      B: 3000
      box_a.shape: torch.Size([24, 3000, 4])
      box_b.shape: torch.Size([3000, 4])
